"""Route class that keeps ordinary client errors out of the exception stream.

Copied verbatim from ``media-api/app/routers/base.py``, which is the canonical version —
it is strictly newer than control-api's and fixes two things control-api's does not: it
branches on ``asyncio.iscoroutinefunction`` so a sync endpoint is not wrapped in an
``async def``, and it returns a bodyless response for statuses where HTTP forbids a body.
Change it there first, then re-copy; do not edit this file in isolation.
"""
